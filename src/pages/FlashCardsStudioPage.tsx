...
import { buildFallbackDsl, type StudyCanvasDsl } from "../study/dsl";
...
export default function FlashCardsStudioPage() {
  ...
  const totalChars = useMemo(
    () => files.reduce((sum, file) => sum + file.chars, 0),
    [files],
  );
  ...
  async function handleGenerate() {
    ...
    try {
      ...
      setDsl(buildFallbackDsl("Local fallback deck. Configure the edge function to get Claude-generated content."));
    } finally {
      setIsLoading(false);
    }
  }
  ...
  return (
    <section className="panel flash-studio-page">
      <header className="page-header">
        <div>
          <p className="eyebrow">Study Tools / Flash Cards Studio</p>
      ...
      <div className="studio-layout">
        ...
        <section className="studio-panel prompt-panel">
          ...
          <button type="button" className="button" onClick={handleGenerate} disabled={isLoading}>
            {isLoading ? "Generating..." : "Generate Study Canvas"}
          </button>
          <p className="small-note">This sends extracted source text to your Supabase Edge Function `claude-study` and then to Claude API.</p>
        ...
        <section className="studio-panel canvas-panel">
          ...
          {dsl ? (
            ...
            <div className="canvas-empty">
              <p>No canvas yet.</p>
              <p>Upload sources and click generate to render an interactive study surface.</p>
            </div>
          )}
        </section>
      </div>
